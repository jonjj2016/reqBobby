            ...
            if (perm.dontSelect) {
              perm.dontSelect.forEach((dont) => {
                if (dont.path === item.path) {
                  if (item.select) {
                    item.select = item.select.split(' ').map((i) => {
...
const authorize = (permissions) => asyncHandler(async (req, res, next) => {
    let isObject = false;
    if (req.query.populate && (req.query.populate.includes('}') || req.query.populate.includes(']'))) {
        isObject = true;
    }
